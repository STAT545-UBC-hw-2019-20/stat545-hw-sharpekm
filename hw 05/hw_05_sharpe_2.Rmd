...
### Drop Oceania from the gapminder dataset
...
<!--- For this exercise, I looked at https://stat545.com/factors-boss.html as well as class materials. ---> 
...
gap_2 <- gapminder %>%
  ...
  droplevels() # Using droplevels() this drops unsed from all factors (including continent and the associated countries))
  #mutate(continent = forcats::fct_drop(continent)) ## using fct_drop() to remove the level from continent 

levels(gap_2$continent)
...
First I am filtering and grouping the dataset and calculating the minimum life expectancy for 2007.
...
I have chosen a plot I made in *assignment #1* to improve. For that assignment, I made a boxplot of life expectancy by continent.
...
- One thing that makes the plot less attractive is that the axes labels are not centered, so I centered them in theme_ridges(). 

- **So, I feel I've improved this graph save for one *MAJOR* issue - it's missing the data for Oceania for some reason unknown to me and sadly I've run out of time to figure it out! So peer reviewers or TA, what obvious mistake am I making/important piece of information am I missing?**
...
### Use ggsave() to save a plot to file. 

I am going to use the "improved plot" (even though the missing Oceania data is hanging over my head) I had in assignment #2 for this exercise. 
...
```{r}
ggsave(here::here("Hw 05", "Hw05 Plot1.jpg"), plot = NEW_plot2)
```

This is what the plot looks like embedded:
...
```{r}
ggsave(here::here("Hw 05", "Hw05 Plot2.jpg"), plot = NEW_plot2, width = 5, height = 6)
```
